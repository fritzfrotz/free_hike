...
| **6** | **Terrain pipeline: windowed GeoTIFF → Terrain-RGB WebP → terrain.pmtiles; contour bake-vs-runtime decision** | ⏳ **IN PROGRESS** — P6.C1–C3 closed: windowed reads → Terrain-RGB WebP → WebMercator bilinear pyramid → terrain.pmtiles (62 tiles z5–12, validated by the app's pmtiles reader); remaining: budget-yield cursor, frontend wiring, contour study |
