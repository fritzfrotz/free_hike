 ...
 *   syncToDropbox(gpx, meta)   → overwrite two files in /Apps/FreeHike/
 *   disconnectDropbox()        → wipe localStorage token
 *
 * Upload model:
 *   Dropbox uses a flat path model — no folder ID step required.
 *   The /Apps/FreeHike/ directory is created implicitly on first upload
...
const DROPBOX_FOLDER_PATH  = '/Apps/FreeHike';
const DROPBOX_TOKEN_KEY    = 'freehike_dropbox_token';
...
/**
 * Writes a single text file to /Apps/FreeHike/<filename> in Dropbox.
